defmodule Cognixir.TextAnalytics do
  ...
  def detect_language(text) when is_binary(text) do
    ...
    case HTTPotion.post(api_base() <> "languages",
           body: encoded_body,
           headers: Cognixir.api_json_header(api_key())
         ) do
  ...
  """
  def detect_topics(documents, stop_words \\ [])
      when is_list(documents) and is_list(stop_words) do
    indexed_documents =
    ...
    case HTTPotion.post(api_base() <> "topics",
           body: encoded_body,
           headers: Cognixir.api_json_header(api_key()),
           timeout: 10000
         ) do
  ...
  def detect_key_phrases(text, language) do
    encoded_body =
      Poison.encode!(%{"documents" => [%{"id" => 1, "text" => text, "language" => language}]})

    case HTTPotion.post(api_base() <> "keyPhrases",
           body: encoded_body,
           headers: Cognixir.api_json_header(api_key())
         ) do
      %HTTPotion.Response{status_code: 200, body: body} ->
  ...
  def detect_sentiment(text, language) do
    ...
    case HTTPotion.post(api_base() <> "sentiment",
           body: encoded_body,
           headers: Cognixir.api_json_header(api_key())
         ) do
      %HTTPotion.Response{status_code: 200, body: body} ->
