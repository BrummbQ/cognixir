defmodule CognixirTest.ComputerVision do
  ...
  test "analyze image url with all options set" do
    result =
      ComputerVision.analyze_image(
        image_url(),
        %ComputerVision.AnalyzeOptions{
          language: "en",
          visualFeatures: "Faces,Color",
          details: "Celebrities"
        }
      )
  ...
  test "describe image with max 3 descriptions" do
    ...
    assert result |> elem(1) |> Map.get("description") |> Map.get("captions") |> Enum.count() ===
             3
  end
  ...
  test "recognize image text with options" do
    result =
      ComputerVision.recognize_character(ocr_image_url(), %ComputerVision.OCROptions{
